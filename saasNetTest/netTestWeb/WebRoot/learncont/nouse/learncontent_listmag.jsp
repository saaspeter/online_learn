<!-- 这个版本带有toggle menu的功能，每次只显示一个菜单的子菜单 -->
...
<html:html lang="true">
      ...
      <ul id="navcate">
         ...
         <li>
            ...
            <a href="javascript:void(0);" 
               class="<%if(datavo.getSublist()!=null&&datavo.getSublist().size()>0){ if(datavo.getPk().equals(pidVar)||datavo.getPk().equals(contentcateidVar)){out.print("collapseCate");}else{out.print("expendCate");} } %>" 
               style="background-color:#DCEAFC;" 
               onclick="gochapter('<%=datavo.getContentcateid() %>');toggleFolder(this, '<%=i %>');return false;">
            ...
            <ul id="cate_ul_<%=i %>" style="display: <% if(datavo.getSublist()!=null&&datavo.getSublist().size()>0&&(datavo.getPk().equals(pidVar)||datavo.getPk().equals(contentcateidVar))){out.print("block");}else{out.print("none");} %>">
  ...
  <div id="DataTableDiv">
    <table id="learnDataTableId" class="listDataTable" >
	  ...
	  <logic:iterate id="vo" name="learncontentForm" property="usecatevo.learncontentlist" indexId="ind">
      <tr id='<%="learnDataTrId"+ind %>' class='<%=(ind%2==0)?"oddRow":"evenRow" %>'>
        <td>
           ...
           <br><font style="margin-left: 30px;font-size: smaller;">(<bean:write name="vo" property="lastupdatetime" format="yyyy-MM-dd"/> 更新)</font>
        </td>
