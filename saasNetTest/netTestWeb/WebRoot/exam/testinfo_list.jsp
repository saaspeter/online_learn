...
<html:html lang="true">
  <head>
    ...
    <bean:define id="userorgidLong" name="testinfoForm" property="userorgid" type="java.lang.Long"></bean:define>
  ...
  <body>
  <div class="listPage">
  <html:form styleId="list" action="/exam/listTestinfo.do" method="post">
  ...
  <div style="height:auto; width:100%;">
     <ul class="tabnav">
	    ...
	    <li><a href="listTodoTestinfo.do?productbaseid=<%=productId %>">需要管理的考试</a></li>
		<li class='selectTab'><a href="javascript:void(0)">查询所有考试</a></li>
	 </ul>
  </div>
  ...
  <div id="firstLineDiv">
      <div id="searchDiv">
		...
		<html:select name="testinfoForm" property="queryVO.teststatuspage">
		   ...
		   <html:optionsSaas consCode="netTest.TestinfoConstant.Teststatusshow"/>
        </html:select>
		<input type="submit" name="Submit" value="<bean:message key="netTest.commonpage.query"/>" />
		<img alt="" src="../styles/imgs/more.gif" onclick="changeComplexSearch('complexSearchDiv');" style="cursor: pointer;">
      </div>
  </div>
  ...
  <div id="complexSearchDiv">
      <table class="complexSearchTable">
          <tr>
              <td>考试名称: 
		          <input type="text" name="queryVO.testname" value="<bean:write name="testinfoForm" property="queryVO.testname"/>"/>
              </td>
              <td>考试创建单位:
                  <input id="createorgnameId" type="text" name="queryVO.createorgname" value="<bean:write name="testinfoForm" property="queryVO.createorgname"/>" readonly="readonly" onclick="selOrg()" />
          ...
          <tr>
              <td>查询级别:
                  <html:select name="testinfoForm" property="queryVO.qrytesttype">
                      <html:optionsSaas consCode="netTest.TestinfoConstant.Qrytesttype"/>
                  </html:select>
              </td>
              <td>考试开放类型:
                  <html:select name="testinfoForm" property="queryVO.opentype">
                      <html:option value=""></html:option>
  ...
  <div id="DataTableDiv">
    <table class="listDataTable" >
      ...
      <tr class="listDataTableHead">
        ...
        <td>考试名</td>
        <td>考试时间</td>
        <td style="width: 100px;">考试状态</td>
        <td>操作<img src="../styles/imgs/details.png" title="考试所在单位管理员才可删除"/></td>
      </tr>
      </thead>
      ...
      <tr class="<%=(ind%2==0)?"oddRow":"evenRow" %>">
        ...
        <td><bean:writeDate name="vo" property="teststartdate" format="yyyy-MM-dd HH:mm"/>&nbsp;--&nbsp;<br/>
	        <bean:writeDate name="vo" property="testenddate" format="yyyy-MM-dd HH:mm" showtimezone="true"/></td>
        <td><bean:writeSaas name="vo" property="teststatus" consCode="netTest.TestinfoConstant.Teststatus"/></td>
        <td>
            <logic:equal name="vo" property="createorgid" value="<%=userorgidLong.toString() %>">
  ...
  <script type="text/javascript">
	 <!-- 
       window.onload=function(){
		    changeComplexSearch("complexSearchDiv","<bean:write name="testinfoForm" property="complexSearchDivStatus"/>");
	        Calendar.setup({
