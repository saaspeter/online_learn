   ...
   function goRightUrl(mainUrl){
	...
	function delRec(formId,checkboxName,url,errHasRef){
        ...
        if(confirm(getMessage("confirmDeleteMsg"))){
            ...
            if(rtnObj.result=="1"){ // forward the success page,if success
               ...
               formObj.action = forwardUrl+delimer+"bundle=BaseKey&DisplayMessageKey=commonWeb.page.pubs.message.jsp.deleteSuccess&DisplayMessageArg0Key="
                                +rtnObj.info;
               formObj.submit();
	...
	function delSingleRecAjax(url, gobackurlId){
		if(url==null||url.length==0){
		    return ;
		}
		url = doContextUrl(url);
		if(confirm(getMessage("confirmDeleteMsg"))){
            ...
            if(rtnObj.result=="1"){ // forward the success page,if success
               ...
               if((typeof(backUrlObj)!="undefined") 
                  ...
                  var backUrl = backUrlObj.value+delimer+"bundle=BaseKey&DisplayMessageKey=commonWeb.page.pubs.message.jsp.deleteSuccessCommon";
	              goUrl(backUrl);
               }
            }else {
               alert(rtnObj.info);
	...
	function clearMessagePage(){
	  ...
	  function newDiv(url,lock,hastop,width,height,posLeft,posTop,divId){ 
		  newDiv_base(0,url,lock,hastop,width,height,posLeft,posTop,divId);
	  }
	  
	  // 屏蔽母窗口和解锁母窗口的函数
	  function newDiv_base(ajaxload,url,lock,hastop,width,height,posLeft,posTop,divId){ 
		  ...
		  var topcode = "";
		  if(hastop!=null&&hastop=="1"){
		     topcode = "<div  style=\"width:100%;height:22px;background:#0099ff;text-align:right;cursor:move;\">"+
		           "<a href=\"\" style=\"text-decoration:none;padding:3px;font-size:15px;\" onclick=\"clearDiv();return false;\"><font>X</font></a>"+"</div>";
		  }
		  ...
		  var strHtml = topcode +
		      ...
		      +"</div>";  
		  if(ajaxload==0){
			  strHtml = strHtml+"<iframe id=\"contentDivFrame\" style=\"margin:0;\" width=\"100%\" height=\""+height+"\" font-size:15px;\" frameborder=0 src=\""+url+"\"></iframe>";
			            +"</div>";
		  }else if(ajaxload==1){
			  strHtml = strHtml+"</div>"; 
		  ...
		  for(var i=1;i<=totalNum;i++){
		     if(i==no){
		        ...
		        if(typeof(url)!='undefined' && url!=null && url!=''){
		        	loadDivByAjax("content"+i, url, true, param);
		        }
		     }else{
