...
function CLASS_CHECK(){
    ...
    me.checkElementInternal = function(element, showmsg){
		...
		// 取得字段的值
		var sVal = getValueCount(element);
		// 取得用户自定的正则表达式Exp
	...
	function getValueCount(el){
		...
		switch(sType){
			...
			case "checkbox":
			case "radio": return getRadioCount(el);
			case "select-one":
			case "select-multiple": return getSelectCount(el);
		}
		//取得radio,checkbox的选中数,用"0"来表示选中的个数,我们写正则的时候就可以通过0{1,}来表示选中个数
		function getRadioCount(el){
			var sValue = "";
			//取得第一个元素的name,搜索这个元素组
   ...
   function submitForm(formId, checkform){
       ...
       if(formObj!=null){
    	  ...
    	  newDiv('/pubs/saveDiv.html',1,0,180,150);
          formObj.submit();
          return true;
       }else{
          return false;
