...
function WebFXLoadTree(sPk, sText, sXmlSrc, sChildNum, sAction, sBehavior, sIcon, sOpenIcon) {
	// call super
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sPk, sText, sAction, sBehavior, sIcon, sOpenIcon);
...
function WebFXLoadTreeItem(sPk, sText, sXmlSrc, sChildNum, sAction, eParent, sIcon, sOpenIcon) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sPk, sText, sAction, eParent, sIcon, sOpenIcon);
...
function _startLoadXmlTree(sSrc, jsNode) {
	if (jsNode.loading || jsNode.loaded)
		return;
	// change by peter.
	// if the src is null,the find the treeUrl defined in the page and add pk
	if(sSrc==null||sSrc==""){
		if((typeof(treeUrl)!="undefined")&&(treeUrl!=null)){
			sSrc = treeUrl+jsNode.pk;
		}
	}
	jsNode.loading = true;
...
function _xmlTreeToJsTree(oNode) {
	...
	var childNum = oNode.getAttribute("childNum");
	// create jsNode
	var jsNode;
	// change by peter
	if (childNum != null && childNum != "" && parseInt(childNum)>0)
		jsNode = new WebFXLoadTreeItem(pk, text, src, childNum, action, parent, icon, openIcon);
	else
		jsNode = new WebFXTreeItem(pk, text, action, parent, icon, openIcon);
