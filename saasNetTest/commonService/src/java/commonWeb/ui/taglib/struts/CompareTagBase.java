...
public abstract class CompareTagBase extends ConditionalTagBase {
    ...
    private static final int INTEGER_COMPARE = 3;
    
    private static final int SHORT_COMPARE = 4;
    ...
    protected boolean condition(int desired1, int desired2)
	    ...
	    }else if (type == INTEGER_COMPARE) {
	        try {
	            long intVariable = Integer.parseInt(variable.toString());
	            if (intVariable < intValue)
	                result = -1;
	            else if (intVariable > intValue)
	                result = +1;
	        } catch (NumberFormatException e) {
	            result = variable.toString().compareTo(value);
	        }
	    }else if (type == SHORT_COMPARE) {
	        try {
	            long shortVariable = Short.parseShort(variable.toString());
	            if (shortVariable < shortValue)
	                result = -1;
	            else if (shortVariable > shortValue)
	                result = +1;
	        } catch (NumberFormatException e) {
	            result = variable.toString().compareTo(value);
	        }
