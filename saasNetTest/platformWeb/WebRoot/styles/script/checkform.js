...
//字符串->正则表达式,不区分大小写
	...
	/*
	 *	显示错误
	 *  errmsgArr: 每个数组元素都又是一个数组，每个子数组包括3个元素，其中第3个元素是要现实的信息
	 */
	function showMessage(errmsgArr){
		//外接显示错误函数
    ...
    me.checkElementInternal = function(element, showmsg){
		...
		if(typeof(sUsage)!="undefined"&&sUsage!=null){
            ...
            if(typeof(strRegArr)!="undefined"&&strRegArr!=null){
               for(j=0;j<strRegArr.length;j++){
               	  if(strRegArr[j]!=null&&strRegArr[j]!=""){
               	  	 var strRegArrPara = strRegArr[j].split(":");
               	  	 if(aUsage[strRegArrPara[0]]!=null){
               	  	 	var rtnArr = checkExpr(strRegArrPara,sVal,checkEmpty);
               	  	 	if(rtnArr!=null&&rtnArr.length>1&&rtnArr[0]=="0"){
               	  	 		fieldPass = false;
							...
							// 如果用户设置了tip，则显示用户的tip，并且只显示一次
                                ...
                                setFlag = true;
								break;
							}else if((sTip==null||sTip.length==0)&&(rtnArr[1]!=null)){
...
//		}
   ...
   function checkElement(element, showmsg){
   	  return g_check.checkElementInternal(element, showmsg);
   }
