...
public class PaperquesDaoImpl extends BaseDao implements PaperquesDao {
    ...
    public Paperques qryPaperquesByQues(Long quesid, Long paperid){
		if(quesid==null || paperid==null)
			return null;
		Paperques vo = new Paperques();
		vo.setQuesid(quesid);
		vo.setPaperid(paperid);
		List<Paperques> list = this.queryForList("Paperques.qryPaperquesByOneQuesID", vo);
		if(list!=null && list.size()>0){
			return list.get(0);
		}else {
			return null;
    ...
    public List<Paper> qryPaperByQues(List<Long> quesidlist, Long shopid){
		...
		if(queslist!=null && queslist.size()>0){
			...
			for(Paperques votemp : queslist){
				if(map.get(votemp.getPaperid())==null){
				   papervotemp = paperdao.selectByPK_plain(votemp.getPaperid());
				   paperlist.add(papervotemp);
				   map.put(votemp.getPaperid(), votemp.getPaperid());
				}
			}
		}
		return paperlist;
    ...
    public List<Question> selectByPid(Long pid, Float paperquesscore){
    	List<Question> paperquesList = null;
    	List<Question> queslist = questionDao.selectByPid(pid);
    	if(queslist!=null){
    		paperquesList = new ArrayList<Question>();
    		for(Question quesvo : queslist){
    			Paperques paperquesvo = new Paperques();
    			paperquesvo.copyQuestion(quesvo);
    			paperquesvo.setPaperquesscore(paperquesscore);
