...
<html:html lang="true">
  <head>
    <html:base />
    <title><bean:message key="acegi.page.{[#folder#]}.user_role_list.jsp.title"/></title>
  ...
  <body>
  <div class="listPage">
  <html:form styleId="list" action="/{[#folder#]}/user_role.do?method=list" method="post">
  ...
  <div id="firstLineDiv">
      ...
      <div id="searchDiv"><bean:message key="acegi.page.{[#folder#]}.user_role.{[#searchColumn#]}"/>: 
		<input type="text" name="queryVO.{[#searchColumn#]}" value="<bean:write name="user_roleForm" property="queryVO.{[#searchColumn#]}"/>"/>
  ...
  <div id="functionLineDiv">
	  <div id="functionButtonDiv">
		  <ul>
			 <li><button onclick="addRecord('/{[#folder#]}/user_role.do?method=addPage');return false;"><bean:message key="acegi.commonpage.add"/></button></li>
			 <li><button onclick="modifyRecord('list','keys','/{[#folder#]}/user_role.do?method=editPage&editType=<%=WebConstant.editType_edit %>&queryVO.roleId=','<bean:message key="commonWeb.js.pageAction.modifyRecord.selectOneMsg" bundle="BaseKey"/>','<bean:message key="commonWeb.js.pageAction.modifyRecord.oneOnlyMsg" bundle="BaseKey"/>');return false;"><bean:message key="acegi.commonpage.modify"/></button></li>
			 <li><button onclick="deleteRecord('list','keys','/{[#folder#]}/user_role.do?method=delete','<bean:message key="commonWeb.js.pageAction.deleteRecord.selectOneMsg" bundle="BaseKey"/>','<bean:message key="commonWeb.js.pageAction.deleteRecord.confirmDeleteMsg" bundle="BaseKey"/>');return false;"><bean:message key="acegi.commonpage.delete"/></button></li>
  ...
  <div id="DataTableDiv">
    <table class="listDataTable" >
      <tr class="listDataTableHead">
        ...
        <td><bean:message key="acegi.page.{[#folder#]}.user_role.userId"/></td>

        <td><bean:message key="acegi.page.{[#folder#]}.user_role.usetype"/></td>
      ...
      <tr>
        <td><input type="checkbox" name="keys" value="<bean:write name="vo" property="roleId"/>" onClick="selectInfo(this,'clickedRow')"></td>
        <a href="javascript:newDiv('/{[#folder#]}/user_role.do?method=editPage&queryVO.roleId=<bean:write name="vo" property="roleId"/>&editType=<%=WebConstant.editType_view %>',0,1,600,400);"><bean:write name="vo" property="{[#searchColumn#]}"/></a>
        <td><bean:write name="vo" property="userId"/></td>

        <td><bean:write name="vo" property="usetype"/></td>
  ...
  <script language=JavaScript>
	 <!--
       window.onload=function(){
		    changeComplexSearch("complexSearchDiv","<bean:write name="user_roleForm" property="complexSearchDivStatus"/>");
	        setListTableStyle();
	   }
     //-->
  </script>
