	...
	<html:form styleId="editForm" action="/{[#folder#]}/save{[#className#]}.do" method="post">
	 ...
	 <input type="hidden" name="vo.{[#pkName#]}" value="<bean:write name="{[#actionName#]}Form" property="vo.{[#pkName#]}"/>">
	  ...
	  <div id="functionBarButtomDiv">
	  	 <ul>
		    <li><button type="button" onclick="submitForm('editForm');"><bean:message key="{[#Project#]}.commonpage.save"/></button></li>
			<li><button type="button" onclick="return false;"><bean:message key="{[#Project#]}.commonpage.reset"/></button></li>
			<li><button type="button" onclick="getAndToUrl('backUrl');return false;"><bean:message key="{[#Project#]}.commonpage.back"/></button></li>
		 </ul>
	  </div>
